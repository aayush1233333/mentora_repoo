    ...
    <div className="min-h-screen flex bg-gray-950">
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-12 bg-gradient-to-br from-indigo-900 via-indigo-800 to-violet-900">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-white/10"><Brain size={24} className="text-white" /></div>
          <span className="text-white font-bold text-xl">Mentora</span>
        </div>
        <div>
          <h2 className="text-4xl font-bold text-white leading-tight mb-4">
            Understand your mind.<br />Protect your energy.
          </h2>
          <p className="text-indigo-200 text-lg">
            Real-time cognitive fatigue tracking powered by AI facial analysis.
          </p>
          <div className="mt-8 grid grid-cols-3 gap-4">
            {[["🧠","AI Powered"],["🔒","Privacy First"],["📊","Deep Insights"]].map(([e,l]) => (
              <div key={l} className="bg-white/10 rounded-xl p-4 text-center">
                <div className="text-2xl mb-1">{e}</div>
                <div className="text-xs text-indigo-200 font-medium">{l}</div>
              </div>
            ))}
          </div>
        </div>
        <p className="text-indigo-300 text-sm">© 2026 Mentora. All rights reserved.</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          ...
          <button onClick={handleGoogle}
            className="w-full py-3 rounded-xl bg-white hover:bg-gray-100 text-gray-800 font-semibold text-sm flex items-center justify-center gap-2 transition-colors">
